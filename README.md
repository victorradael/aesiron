...
2.  **Orquestração Massiva**:
    ...
    aesiron dns-setup    # Configura apenas esta maquina para resolver *.iron via /etc/hosts
    aesiron dns-reset    # Remove as entradas locais do Aesiron do /etc/hosts
    ```

3.  **DNS da Rede Local**:
    O Aesiron pode expor os apps por hostnames como `http://meu-app.iron`, desde que os dispositivos da sua rede usem o DNS da maquina onde o Aesiron esta rodando.

    Para configurar apenas a maquina atual sem alterar o DNS global do sistema:
    ```bash
    aesiron dns-setup
    ```

### Arquitetura da Rede Interna

```mermaid
flowchart LR
    Cliente[Cliente na LAN]
    DNSLocal[Resolvedor local ou cliente configurado]
    DNS[Aesiron DNS\ndnsmasq :53]
    Gateway[Aesiron Gateway\nnginx :80]
    App1[app-aesiron-xpto\nStreamlit :8501]
    App2[app-aesiron-xpto2\nStreamlit :8502]

    Cliente -->|consulta xpto.iron| DNSLocal
    DNSLocal -->|usa DNS configurado| DNS
    DNS -->|responde IP LAN do host| Cliente
    Cliente -->|HTTP Host: xpto.iron| Gateway
    Gateway -->|proxy reverso| App1
    Cliente -->|HTTP Host: xpto2.iron| Gateway
    Gateway -->|proxy reverso| App2
```

- `dnsmasq` responde qualquer `*.iron` com o IP LAN da maquina onde o Aesiron roda para clientes da rede que apontarem para esse DNS.
- `nginx` recebe a requisicao HTTP e escolhe o app certo pelo hostname.
- `aesiron dns-setup` nao troca o DNS global da maquina; ele escreve apenas entradas gerenciadas no `/etc/hosts` para os apps ativos.
- os containers de DNS e gateway sao efemeros e so existem quando ha apps ativos para expor.
- os arquivos em `.aesiron-infra/` sao gerados dinamicamente e nao devem ser versionados.
